...
qmckl_exit_code qmckl_distance_device (
  ...
  if ( (transab,1) == 0 && LDB < 3 ) {
     return info = QMCKL_INVALID_ARG_9;
  }
...
,*/
  ...
  case 0:
     for(int j=0; j<n; n++) {
        for(int i=0, i<m; m++) {
		   x = A[i*lda] - B[j*ldb];
           y = A[i*lda + 1] - B[j*ldb + 1];
           z = A[i*lda + 2] - B[j*ldb + 2];
		   C[i + j*ldc] = sqrt(x*x + y*y + z*z);
        }
     }
     break;
  ...
  case 3:
     for(int j=1, j<n; j++) {
        for(int i=1, i<m; i++) {
           x = A[i]       - B[j];
           y = A[i+lda]   - B[j+ldb];
           z = A[i+2*lda] - B[j+2*lda];
           C[i,j] = sqrt(x*x + y*y + z*z);
        }
     }
     break;
