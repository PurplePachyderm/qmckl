                                ...
                                const int64_t size_max)
{
  ...
  // This accepts a host pointer an copies it in context as a device pointer
  ...
  rc = qmckl_vector_of_double_device(context, charge, num, &(ctx->nucleus.charge));
  if (rc != QMCKL_SUCCESS) {
    return qmckl_failwith( context,
                           QMCKL_FAILURE,
                           "qmckl_set_nucleus_charge",
                        ...
                        const int64_t size_max)
{
  ...
  // TODO qmckl_matrix_of_double for device
  if (transp == 'N') {
    qmckl_matrix At;
...
#ifdef HAVE_DEVICE_POINTERS
interface
  integer(c_int32_t) function qmckl_set_nucleus_coord_device(context, transp, coord, size_max) &
    bind(C)
    use, intrinsic :: iso_c_binding
    import
    implicit none
