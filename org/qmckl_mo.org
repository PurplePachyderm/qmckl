...
#ifdef HAVE_DEVICE_POINTERS
qmckl_exit_code  qmckl_set_mo_basis_coefficient_device (qmckl_context context, const double  * coefficient);
#endif
   #+end_src
...
qmckl_exit_code  qmckl_set_mo_basis_coefficient_device(qmckl_context context, const double* coefficient) {
  ...
  qmckl_memory_info_struct mem_info = qmckl_memory_info_struct_zero;
  mem_info.size = ctx->ao_basis.ao_num * ctx->mo_basis.mo_num * sizeof(double);
  double* new_array = (double*) qmckl_malloc_device(context, mem_info, 0);
  if (new_array == NULL) {
    return qmckl_failwith( context,
                           QMCKL_ALLOCATION_FAILED,
                           "qmckl_set_mo_basis_coefficient",
                                   ...
                                   double* restrict const mo_vgl )
{
    ...
    int64_t * idx = malloc(sizeof(int64_t)*ao_num*point_num);
    double * av1 = malloc(sizeof(double)*ao_num*point_num);
    double * av2 = malloc(sizeof(double)*ao_num*point_num);
    double * av3 = malloc(sizeof(double)*ao_num*point_num);
    double * av4 = malloc(sizeof(double)*ao_num*point_num);
    double * av5 = malloc(sizeof(double)*ao_num*point_num);
  ...
  #pragma omp target enter data map(to:coef_normalized_t[0:ao_num*mo_num],\
                                ...
                                map(alloc:\
                                    idx[0:ao_num*point_num],\
                                    av1[0:ao_num*point_num],\
                                    av2[0:ao_num*point_num],\
                                    av3[0:ao_num*point_num],\
                                    av4[0:ao_num*point_num],\
                                    av5[0:ao_num*point_num]\
                                    )
	{
  ...
  for (int64_t ipoint=0 ; ipoint < point_num ; ++ipoint) {
    ...
    vgl5 =  vgl1 + (mo_num << 2);
    const double * avgl1 = &(ao_vgl[ipoint*5*ao_num]);
    const double * avgl2 = avgl1 + ao_num;
    const double * avgl3 = avgl1 + (ao_num << 1);
    const double * avgl4 = avgl1 + (ao_num << 1) + ao_num;
    const double * avgl5 = avgl1 + (ao_num << 2);
    ...
    for (int64_t k=0 ; k<ao_num ; ++k) {
      if (avgl1[k] != 0.) {
        idx[ipoint*ao_num + nidx] = k;
        av1[ipoint*ao_num + nidx] = avgl1[k];
        av2[ipoint*ao_num + nidx] = avgl2[k];
        av3[ipoint*ao_num + nidx] = avgl3[k];
        av4[ipoint*ao_num + nidx] = avgl4[k];
        av5[ipoint*ao_num + nidx] = avgl5[k];
        ++nidx;
      }
    }
    ...
    for (n=0 ; n < nidx-4 ; n+=4) {
      const double * ck1 = coef_normalized_t + idx[ipoint*ao_num + n  ]*mo_num;
      const double * ck2 = coef_normalized_t + idx[ipoint*ao_num + n+1]*mo_num;
      const double * ck3 = coef_normalized_t + idx[ipoint*ao_num + n+2]*mo_num;
      const double * ck4 = coef_normalized_t + idx[ipoint*ao_num + n+3]*mo_num;

      double a11 = av1[ipoint*ao_num + n  ];
      double a21 = av1[ipoint*ao_num + n+1];
      double a31 = av1[ipoint*ao_num + n+2];
      double a41 = av1[ipoint*ao_num + n+3];

      double a12 = av2[ipoint*ao_num + n  ];
      double a22 = av2[ipoint*ao_num + n+1];
      double a32 = av2[ipoint*ao_num + n+2];
      double a42 = av2[ipoint*ao_num + n+3];

      double a13 = av3[ipoint*ao_num + n  ];
      double a23 = av3[ipoint*ao_num + n+1];
      double a33 = av3[ipoint*ao_num + n+2];
      double a43 = av3[ipoint*ao_num + n+3];

      double a14 = av4[ipoint*ao_num + n  ];
      double a24 = av4[ipoint*ao_num + n+1];
      double a34 = av4[ipoint*ao_num + n+2];
      double a44 = av4[ipoint*ao_num + n+3];

      double a15 = av5[ipoint*ao_num + n  ];
      double a25 = av5[ipoint*ao_num + n+1];
      double a35 = av5[ipoint*ao_num + n+2];
      double a45 = av5[ipoint*ao_num + n+3];
    ...
    const int64_t n0 = n < 0 ? 0 : n;
    for (int64_t m=n0 ; m < nidx ; m+=1) {
      const double * ck = coef_normalized_t + idx[ipoint*ao_num + m]*mo_num;
      double a1 = av1[ipoint*ao_num + m];
      double a2 = av2[ipoint*ao_num + m];
      double a3 = av3[ipoint*ao_num + m];
      double a4 = av4[ipoint*ao_num + m];
      double a5 = av5[ipoint*ao_num + m];
  ...
  #pragma omp target update from(mo_vgl[0:point_num*5*mo_num])
  }
                                   ...
                                   double* restrict const mo_vgl )
{
  ...
  #pragma acc data copyin(coef_normalized_t[0:ao_num*mo_num],\
                          ao_vgl[0:point_num*5*ao_num])\
	                 copy(mo_vgl[0:point_num*5*mo_num])\
                   create(idx[0:ao_num*point_num],\
                          av1[0:ao_num*point_num],\
                          av2[0:ao_num*point_num],\
                          av3[0:ao_num*point_num],\
                          av4[0:ao_num*point_num],\
  ...
  for (int64_t ipoint=0 ; ipoint < point_num ; ++ipoint) {
    ...
    for (n=0 ; n < nidx-4 ; n+=4) {
      ck1 = coef_normalized_t + idx[ipoint*ao_num + n  ]*mo_num;
      ck2 = coef_normalized_t + idx[ipoint*ao_num + n+1]*mo_num;
      ck3 = coef_normalized_t + idx[ipoint*ao_num + n+2]*mo_num;
      ck4 = coef_normalized_t + idx[ipoint*ao_num + n+3]*mo_num;
    ...
    const int64_t n0 = n < 0 ? 0 : n;
    for (int64_t m=n0 ; m < nidx ; m+=1) {
      ck = coef_normalized_t + idx[ipoint*ao_num + m]*mo_num;
      double a1 = av1[ipoint*ao_num + m];
      double a2 = av2[ipoint*ao_num + m];
      double a3 = av3[ipoint*ao_num + m];
      double a4 = av4[ipoint*ao_num + m];
