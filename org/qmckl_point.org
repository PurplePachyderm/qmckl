...
#ifdef HAVE_DEVICE_POINTERS
qmckl_exit_code
qmckl_set_point_device (qmckl_context context,
                        const char transp,
                        const double* coord,
                        const int64_t num,
                        int device_id)
{
  ...
  qmckl_exit_code rc;
  if (ctx->point.num < num) {
    printf("Need new alloc\n");
    if (ctx->point.coord.data != NULL) {
      printf("Free previous ctx->point.coord\n");
    ...
    printf("ctx->point.coord after alloc = %p\n", ctx->point.coord);
    if (ctx->point.coord.data == NULL) {
      return qmckl_failwith( context,
                             QMCKL_ALLOCATION_FAILED,
                             "qmckl_set_point",
  ...
  if (transp == 'T') {
    ...
    printf("elec_coord before kernel (coord) = %p\n", coord);
    printf("ctx->point.coord before kernel (a) = %p\n", a);


    // DEVICE POINTER
    #pragma omp target is_device_ptr(coord, a)
    {
    for (int64_t i=0 ; i<3*num ; ++i) {
      a[i] = coord[i];
    }
